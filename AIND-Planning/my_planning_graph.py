...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for a in self.all_actions:
            #build action node for this actions
            a_node = PgNode_a(a)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Make sure node_a1's effect_add aren't canceled by node_a2's  effects_rem
        for e in node_a1.action.effect_add:
            if e in node_a2.action.effect_rem:
                return True
        # Make sure node_a2's effect_add aren't canceled by node_a1's  effects_rem
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Make sure node_a1's effect_add aren't canceled by node_a2's precond_neg
        for e in node_a1.action.effect_add:
            if e in node_a2.action.precond_neg:
                return True
        # Make sure node_a2's effect_add aren't canceled by node_a1's  precond_neg
        for e in node_a2.action.effect_add:
            if e in node_a1.action.precond_neg:
                return True

        # Make sure node_a1's effect_rem aren't canceled by node_a2's  precond_pos
        for e in node_a1.action.effect_rem:
            if e in node_a2.action.precond_pos:
                return True
        # Make sure node_a2's effect_rem aren't canceled by node_a1's  precond_pos
